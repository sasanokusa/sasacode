...
| コアの行数（ai + agent + plugin-api + tools） | 約 4,000 行（当初 3,000 行から改訂） | 3,306 行（v0.7.0） |
| テスト | LLM なしで E2E | 94 テスト（18 ファイル）。replayProvider と、テスト用の MCP サーバー、偽の browsr-agent、偽の Models API を使う。組み込み Skill `tool-authoring` のコード例も抜き出して実行する |
