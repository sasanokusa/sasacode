...
| ctrl+c ×2 / ctrl+d | 終了 |
| `/model` `/resume` `/clear` `/help` `/permission` `/fork` `/session` `/exit`（`/quit`） | コアのスラッシュコマンド |
...
| 名前 | 内容 |
| --- | --- |
| `tool-repair` | 小型モデルの壊れたツール呼び出しを、検証の前に修復する：JSON（末尾カンマ、閉じ括弧、クォートのないキー、`True`/`None`、コードフェンス、二重エンコード）、キー名（`file` → `path`）、型（`"20"` → `20`）、ツール名の typo。候補が1つに絞れるときだけ直す。直した内容は tool_result の先頭でモデルに短く伝え、履歴には修復後の呼び出しを残し、元の出力はセッションに `tool_repair` として記録する |
| `repetition-guard` | 生成中に同じ文を繰り返し始めたら止め、繰り返し部分を1つだけ残して、モデルに続きを促す。ツール引数（ファイルの中身など）はより緩い基準で見る。`samplingAfterStop` を設定すると、止めた次のリクエストだけ penalty などを変えられる |
...
`~/.sasacode/sessions/<cwd>/<日時>_<id>.jsonl` にメッセージを1件ずつ追記保存する。セッション ID は TUI のフッターと `/session` に表示され、終了時には `再開: sasacode -r <id>` を出す。ヘッドレスでは、テキスト出力の最後に stderr へ、JSONL 出力の先頭に `{"type":"session"}` として出す。`sasacode -r <id> -p "…"` を繰り返せば、tmux のない環境でも会話を続けられる。`/fork` を使うと、過去の任意のメッセージの直前から新しいセッションに分岐できる。書きかけの行や、結果が揃っていないツール呼び出しは再開時に切り捨て、最後に完了したターンから再開する。使用中の API キーは書き込み前に `[REDACTED]` に置換する。`SASACODE_HOME` で `~/.sasacode` の場所を変えられる。
